---------------------------------------------------------------------------------
--                                                                             --
-- PoliteWhisper.lua - Main code for Polite Whisper Wrath Extended WoW add-on. --
--                                                                             --
---------------------------------------------------------------------------------

-- Version info:

-- Updated Polite Whisper to include:
-- 	+ All WoTLK Instance and Heroics
--  + Raised max search level to 80
--  + Added Death Knights as searchable Class
-- by ][odine

-- $Date: 2009-02-2 (Tuesday, 2 Feb 2009) $ Ver 1.1
	-- Added Class coloring
	-- Removed name coloring temporarily
-- $Date: 2009-02-1 (Monday, 1 Feb 2009) $ Ver 1.0
	-- Released
...
function PoliteWhisper_Invoke()
		...
		then
				...
				UIDropDownMenu_SetWidth(PoliteWhisper_DungeonDrop, 255);
				UIDropDownMenu_Initialize(PoliteWhisper_OtherDrop,
						PoliteWhisper_OtherInit);
				UIDropDownMenu_SetWidth(PoliteWhisper_OtherDrop, 140);
...
function PoliteWhisper_OnLoad()
		...
		T[PW_DEATHKNIGHT] = {PoliteWhisper_DeathknightDrop, PoliteWhisper_DeathknightInit};
		...
		do
				...
				UIDropDownMenu_SetWidth(T[1], 70);
...
function PoliteWhisper_Search()
		...
		if (not Min) or (Min > 80)
		then
				PoliteWhisper_MsgHack(PW_NO_MIN_ERR);
				return;
		end

		if (not Max) or (Max > 80)
		then
...
function PoliteWhisper_ShowItem(i, Name, Items)
		...
		Ctrls.Name:SetTextColor(1, 1, 1, 1);
		-- Set Name to white until fix can be made for party coloring.
		-- Ctrls.Name:SetTextColor(R, G, B, 1);
		if (Items.Class == "Druid")then
			Ctrls.Class:SetTextColor(1.00, 0.49, 0.04, 1);
		elseif 	(Items.Class == "Paladin")then
			Ctrls.Class:SetTextColor(0.96, 0.55, 0.73, 1);
		elseif 	(Items.Class == "Shaman")then
			Ctrls.Class:SetTextColor(0.14, 0.35, 1.00, 1);
		elseif 	(Items.Class == "Death Knight")then
			Ctrls.Class:SetTextColor(0.77, 0.12, 0.23, 1);
		elseif 	(Items.Class == "Hunter")then
			Ctrls.Class:SetTextColor(0.67, 0.83, 0.45, 1);
		elseif 	(Items.Class == "Priest")then
			Ctrls.Class:SetTextColor(1.00, 1.00, 1.00, 1);
		elseif 	(Items.Class == "Warlock")then
			Ctrls.Class:SetTextColor(0.58, 0.51, 0.79, 1);
		elseif 	(Items.Class == "Mage")then
			Ctrls.Class:SetTextColor(0.41, 0.80, 0.94, 1);
...
function PoliteWhisper_SetOptions()
		...
		if (PoliteWhisper_HeroicEnable:GetChecked()) then
			PoliteWhisper_MinLevel:SetText("80");
			PoliteWhisper_MaxLevel:SetText("80");
		end
